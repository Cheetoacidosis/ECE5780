...
volatile uint16_t current_volume = 1;

//BaseType_t LEDstate = 0;
QueueHandle_t xStateQueue;

//Sine lookup table
//const BaseType_t SLUT[64] = {   //ck 4/21
...
const BaseType_t SLUT[64] = {
			0x125, 0x141, 0x15e, 0x179, 0x194, 0x1ae, 0x1c7, 0x1de,
			0x1f3, 0x207, 0x218, 0x226, 0x233, 0x23c, 0x243, 0x248,
			0x249, 0x248, 0x243, 0x23c, 0x233, 0x226, 0x218, 0x207,
			0x1f3, 0x1de, 0x1c7, 0x1ae, 0x194, 0x179, 0x15e, 0x141,
			0x125, 0x108, 0xeb, 0xd0, 0xb5, 0x9b, 0x82, 0x6b,
			0x56, 0x42, 0x31, 0x23, 0x16, 0x0d, 0x06, 0x01,
			0x00, 0x01, 0x06, 0x0d, 0x16, 0x23, 0x31, 0x42,
			0x56, 0x6b, 0x82, 0x9b, 0xb5, 0xd0, 0xeb, 0x108};

// Note lookup table. First is high a, then b... 
const BaseType_t ARR_LUT[25] = {
		0x1DD, //C3
		0x1C2, //C#3
		0x1A9, //D3
		0x190, //D#3
...
//		DAC->DHR12R1 = (uint16_t)(SLUT[SLUT_CNT] * current_volume); //ck 4/21
		DAC->DHR12R1 = SLUT[SLUT_CNT] * current_volume;
...
void change_frequency() {
	uint16_t buffer = 0;
	while(1) {
	// Receive from USART1 queue
		xQueueReceive(freq_peek_queue, &buffer, portMAX_DELAY);
...
void change_volume() {
    ...
    while(1) {
        ...
        // Store globally (or in volatile variable)
        current_volume = scale_int;
    }
}
